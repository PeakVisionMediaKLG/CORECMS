...
$USER->DB = $DB;
$TXT = new TXT($DB);
define('TXT', $TXT->GET_VALUES()); //TXT is defined as all values of the "name" field in DB:Table: be_captions
...
if($LOGIN_FORM_ACTION=="") //no input provided, check if $_SESSION['session_identifier'] is set
{
	...
	DOCUMENT::HEADER(array('title'=>"Login to Core CMS Backend"));
						...
						else
						{
							...
							ROW::PRE(array());
								COLUMN::PRE(array('class'=>'col col-sm-6 text-end'));
									A::PRE(array('class'=>'btn btn-primary mt-5 btn-block','href'=>'../../show.php','caption'=>TXT['Continue']));
									A::POST();
								COLUMN::POST();
