...
include_once('core/classes/class.alert.php');
...
$MY_SESSION->SET_VAL('interface','toolbar_expand_system',0);

$DB = new DB($SETTINGS,$MY_SESSION);
$ALERTS = new ALERT($MY_SESSION);
...
<html lang="<?php echo $PAGE->GET_PAGEVAL('pLanguage'); ?>">
  <head>
    ...
    <?php   echo $DB->SETTINGS['headInclude'];?>  
    <?php   $CONTENT->PRE_INCLUDES(); ?>  
  ...
  <body>
  	...
  	<?php   if($USER->USER_AUTH_OK) { $ADMIN->TOOLBAR(); } //print_r($_SESSION);

            $ALERTS->SHOW_ALERTS();
			 
            $CONTENT->BUILD_CONTENT();

			$CONTENT->ADD_TOPLEVEL_CONTENT();

			if($USER->USER_AUTH_OK) { $ADMIN->TOOLBAR_CLOSETAGS(); } 
      

	?>
	<?php   $CONTENT->POST_INCLUDES(); ?>   
    ...
    <?php   echo $DB->SETTINGS['bodyInclude'];?>  
