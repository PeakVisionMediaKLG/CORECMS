...
$current_status = $DB->PREP_QUERY ('SELECT cActive FROM content WHERE cUID=? LIMIT 1', 
                                     'content', 
                                     array('cUID'),
                                     array($cUID), 
...
if($current_status) $new_status=0; else $new_status=1;    
    ...
    $action = $DB->PREP_QUERY ('UPDATE content SET cActive = ?, cLastEditor = ?, cDate = ? WHERE cUID = ?', 
                                                 'content', 
